...
def forward_prop(data, labels, params):
    ...
    # Hidden layer: a = sigmoid(data @ W1 + b1)
    a = sigmoid(data @ W1 + b1)          # (n, h)

    # Output layer: output = softmax(a @ W2 + b2)
    output = softmax(a @ W2 + b2)        # (n, K)

    # Average cross-entropy loss (clip to avoid log(0))
    n = data.shape[0]
    cost = -np.sum(labels * np.log(output + 1e-10)) / n

    return a, output, cost
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    # Gradient of avg CE loss w.r.t. pre-softmax logits z2: (y_hat - y) / n
    delta2 = (output - labels) / n       # (n, K)

    dW2 = a.T @ delta2                   # (h, K)
    db2 = delta2.sum(axis=0)             # (K,)

    # Backprop through sigmoid hidden layer
    delta1 = (delta2 @ W2.T) * a * (1 - a)  # (n, h)

    dW1 = data.T @ delta1                # (d, h)
...
def backward_prop_regularized(data, labels, params, forward_prop_func, reg):
    ...
    # Add gradient of λ(||W1||² + ||W2||²); biases are not regularized.
    grads['W1'] = grads['W1'] + 2 * reg * params['W1']
    grads['W2'] = grads['W2'] + 2 * reg * params['W2']

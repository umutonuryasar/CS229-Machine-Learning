...
def initial_state():
    ...
    # *** START CODE HERE ***
    # Store beta coefficients and corresponding training examples seen so far.
    # theta = sum_j beta_j * phi(x_j), so theta^T phi(x) = sum_j beta_j * K(x_j, x).
    return {'betas': [], 'xs': []}
    # *** END CODE HERE ***
...
def predict(state, kernel, x_i):
    ...
    """
    # *** START CODE HERE ***
    score = sum(beta * kernel(x, x_i) for beta, x in zip(state['betas'], state['xs']))
    return sign(score)
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    state['betas'].append(learning_rate * (y_i - prediction))
    state['xs'].append(x_i)
    # *** END CODE HERE ***

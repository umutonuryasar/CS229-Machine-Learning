...
def fit_naive_bayes_model(matrix, labels):
    ...
    # Laplace smoothing: add 1 to each word count, add vocab_size to denominator
    spam_counts = matrix[labels == 1].sum(axis=0) + 1
    ham_counts = matrix[labels == 0].sum(axis=0) + 1

    phi_spam = spam_counts / spam_counts.sum()
    phi_ham = ham_counts / ham_counts.sum()

    return (phi_y, phi_spam, phi_ham)
    # *** END CODE HERE ***
...
def predict_from_naive_bayes_model(model, matrix):
    ...
    """
    # *** START CODE HERE ***
    phi_y, phi_spam, phi_ham = model

    log_p_spam = matrix @ np.log(phi_spam) + np.log(phi_y)
    log_p_ham = matrix @ np.log(phi_ham) + np.log(1 - phi_y)
...
def get_top_five_naive_bayes_words(model, dictionary):
    ...
    log_ratio = np.log(phi_spam) - np.log(phi_ham)

    idx_to_word = {idx: word for word, idx in dictionary.items()}
    top_indices = np.argsort(log_ratio)[-5:][::-1]

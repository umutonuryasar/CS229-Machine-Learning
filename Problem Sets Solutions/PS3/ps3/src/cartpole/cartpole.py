...
def choose_action(state, mdp_data):
    ...
    q0 = transition_probs[state, :, 0] @ value
    q1 = transition_probs[state, :, 1] @ value
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for s in range(num_states):
        for a in range(2):
            ...
            if total > 0:
                mdp_data['transition_probs'][s, :, a] = transition_counts[s, :, a] / total

    for s in range(num_states):
        total = mdp_data['reward_counts'][s, 1]
        if total > 0:
            mdp_data['reward'][s] = -mdp_data['reward_counts'][s, 0] / total
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        # Q(s,a) = R(s) + gamma * sum_{s'} P(s'|s,a) V(s')   shape: (num_states, 2)
        # transition_probs[s, s', a], value[s'] -> sum over s' (axis 1)
        q_values = reward[:, np.newaxis] + gamma * np.einsum('ijk,j->ik', transition_probs, value)
        new_value = np.max(q_values, axis=1)

        max_change = np.max(np.abs(new_value - value))
        value = new_value
    ...
    return converged_in_one_iteration
    # *** END CODE HERE ***

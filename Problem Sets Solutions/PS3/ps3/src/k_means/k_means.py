...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    H, W, C = image.shape
    K = centroids.shape[0]
    pixels = image.reshape(-1, C).astype(float)  # (H*W, C)
    new_centroids = centroids.astype(float).copy()

    for it in range(max_iter):
        # Compute distances from each pixel to every centroid: (H*W, K)
        diff = pixels[:, np.newaxis, :] - new_centroids[np.newaxis, :, :]  # (H*W, K, C)
        dist = np.sum(diff ** 2, axis=2)  # (H*W, K)
        assignments = np.argmin(dist, axis=1)  # (H*W,)
        ...
        if (it + 1) % print_every == 0:
            print('[INFO] Iteration {}/{}'.format(it + 1, max_iter))

        if np.allclose(prev_centroids, new_centroids):
            print('[INFO] Converged at iteration {}'.format(it + 1))
            break
    # *** END YOUR CODE ***
...
def update_image(image, centroids):
    ...
    diff = pixels[:, np.newaxis, :] - centroids[np.newaxis, :, :]  # (H*W, K, C)
    dist = np.sum(diff ** 2, axis=2)  # (H*W, K)
    assignments = np.argmin(dist, axis=1)  # (H*W,)

    pixels = centroids[assignments]
    image = pixels.reshape(H, W, C).astype(np.uint8)
    # *** END YOUR CODE ***
